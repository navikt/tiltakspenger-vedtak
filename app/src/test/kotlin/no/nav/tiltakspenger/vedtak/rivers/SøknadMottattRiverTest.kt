...
internal class SøknadMottattRiverTest {
    ...
    @Test
    fun `sende inn en søknad med null i alle felter som skal kunne være null i trygdogpensjon`() {
        testRapid.sendTestMessage(søknadMottattMedNullITrygdOgPensjon())
        with(testRapid.inspektør) {
            assertEquals(1, size)
            assertEquals("behov", field(0, "@event_name").asText())
            assertEquals("personopplysninger", field(0, "@behov")[0].asText())
    ...
    private fun søknadMottattMedNullITrygdOgPensjon(): String =
        """
            {
              "@event_name": "søknad_mottatt",
              "søknad": {
