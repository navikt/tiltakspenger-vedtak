...
internal class AktivitetsloggTest {
    ...
    fun `severe oppdaget og kaster exception`() {
        ...
        assertThrows<no.nav.tiltakspenger.innsending.Aktivitetslogg.AktivitetException> { aktivitetslogg.severe(melding) }
    ...
    fun `Behov kan ha detaljer`() {
        ...
        hendelse1.behov(
            no.nav.tiltakspenger.innsending.Aktivitetslogg.Aktivitet.Behov.Behovtype.personopplysninger,
    ...
    private fun assertAtBådeKontekstOgDetaljerBlirMappetInnIKafkaMeldingene(behov: List<no.nav.tiltakspenger.innsending.Aktivitetslogg.Aktivitet.Behov>) {
    ...
    private fun assertInfo(message: String, aktivitetslogg: no.nav.tiltakspenger.innsending.Aktivitetslogg = this.aktivitetslogg) {
        var visitorCalled = false
        aktivitetslogg.accept(
            object : no.nav.tiltakspenger.innsending.IAktivitetsloggVisitor {
                override fun visitInfo(
                    kontekster: List<no.nav.tiltakspenger.innsending.Kontekst>,
                    aktivitet: no.nav.tiltakspenger.innsending.Aktivitetslogg.Aktivitet.Info,
                    melding: String,
    ...
    private fun assertWarn(message: String, aktivitetslogg: no.nav.tiltakspenger.innsending.Aktivitetslogg = this.aktivitetslogg) {
        var visitorCalled = false
        aktivitetslogg.accept(
            object : no.nav.tiltakspenger.innsending.IAktivitetsloggVisitor {
                override fun visitWarn(
                    kontekster: List<no.nav.tiltakspenger.innsending.Kontekst>,
                    aktivitet: no.nav.tiltakspenger.innsending.Aktivitetslogg.Aktivitet.Warn,
                    melding: String,
                    tidsstempel: LocalDateTime,
                ) {
                    visitorCalled = true
    ...
    private fun assertError(message: String, aktivitetslogg: no.nav.tiltakspenger.innsending.Aktivitetslogg = this.aktivitetslogg) {
        var visitorCalled = false
        aktivitetslogg.accept(
            object : no.nav.tiltakspenger.innsending.IAktivitetsloggVisitor {
                override fun visitError(
                    kontekster: List<no.nav.tiltakspenger.innsending.Kontekst>,
                    aktivitet: no.nav.tiltakspenger.innsending.Aktivitetslogg.Aktivitet.Error,
                    melding: String,
                    tidsstempel: LocalDateTime,
                ) {
                    visitorCalled = true
    ...
    private fun assertSevere(message: String, aktivitetslogg: no.nav.tiltakspenger.innsending.Aktivitetslogg = this.aktivitetslogg) {
        var visitorCalled = false
        aktivitetslogg.accept(
            object : no.nav.tiltakspenger.innsending.IAktivitetsloggVisitor {
                override fun visitSevere(
                    kontekster: List<no.nav.tiltakspenger.innsending.Kontekst>,
                    aktivitet: no.nav.tiltakspenger.innsending.Aktivitetslogg.Aktivitet.Severe,
                    melding: String,
                    tidsstempel: LocalDateTime,
                ) {
                    visitorCalled = true
    ...
    private class TestKontekst(
        ...
        override fun opprettKontekst() = no.nav.tiltakspenger.innsending.Kontekst(
    ...
    private class TestHendelse(
        private val melding: String,
        val logg: no.nav.tiltakspenger.innsending.Aktivitetslogg,
    ) : no.nav.tiltakspenger.innsending.KontekstLogable, no.nav.tiltakspenger.innsending.IAktivitetslogg by logg {
        init {
            logg.addKontekst(this)
        }

        override fun opprettKontekst() = no.nav.tiltakspenger.innsending.Kontekst(kontekstType = "TestHendelse")
