...
import no.nav.tiltakspenger.innsending.Aktivitetslogg.Aktivitet
...
open class Aktivitetslogg(
    private var forelder: no.nav.tiltakspenger.innsending.IAktivitetslogg? = null,
    private val aktiviteter: MutableList<Aktivitet> = mutableListOf(),
) : no.nav.tiltakspenger.innsending.IAktivitetslogg {
    ...
    private val kontekster = mutableListOf<no.nav.tiltakspenger.innsending.KontekstLogable>() // Doesn't need serialization
    ...
    private fun info() =
        Aktivitet.Info.Companion.filter(aktiviteter)
    private fun warn() =
        Aktivitet.Warn.Companion.filter(aktiviteter)
    override fun behov() =
        Aktivitet.Behov.Companion.filter(aktiviteter)
    private fun error() =
        Aktivitet.Error.Companion.filter(aktiviteter)
    private fun severe() =
        Aktivitet.Severe.Companion.filter(aktiviteter)
    ...
    sealed class Aktivitet(
        ...
        class Info(
            ...
            companion object {
                internal fun filter(aktiviteter: List<Aktivitet>): List<Aktivitet.Info> {
                    return aktiviteter.filterIsInstance<Aktivitet.Info>()
                }
            }
        ...
        class Warn(
            ...
            companion object {
                internal fun filter(aktiviteter: List<Aktivitet>): List<Aktivitet.Warn> {
                    return aktiviteter.filterIsInstance<Aktivitet.Warn>()
                }
            }
        ...
        class Behov(
            val type: Aktivitet.Behov.Behovtype,
            kontekster: List<Kontekst>,
            melding: String,
            ...
            companion object {
                internal fun filter(aktiviteter: List<Aktivitet>): List<Aktivitet.Behov> {
                    return aktiviteter.filterIsInstance<Aktivitet.Behov>()
                }
            }
            ...
            enum class Behovtype {
                personopplysninger, skjerming, tiltak, arenaytelser, fpytelser, overgangsstønad, uføre,
            }
        }
        ...
        class Error(
            ...
            companion object {
                internal fun filter(aktiviteter: List<Aktivitet>): List<Aktivitet.Error> {
                    return aktiviteter.filterIsInstance<Aktivitet.Error>()
                }
            }
        ...
        class Severe(
            ...
            companion object {
                internal fun filter(aktiviteter: List<Aktivitet>): List<Aktivitet.Severe> {
                    return aktiviteter.filterIsInstance<Aktivitet.Severe>()
                }
            }
...
interface IAktivitetslogg {
    ...
    fun aktiviteter(): List<Aktivitet>
    fun add(aktivitet: Aktivitet)
}
