...
import no.nav.tiltakspenger.innsending.domene.Aktivitetslogg.Aktivitet
import no.nav.tiltakspenger.innsending.domene.Aktivitetslogg.Aktivitet.Behov
import no.nav.tiltakspenger.innsending.domene.Aktivitetslogg.Aktivitet.Behov.Behovtype
import no.nav.tiltakspenger.innsending.domene.Aktivitetslogg.Aktivitet.Error
import no.nav.tiltakspenger.innsending.domene.Aktivitetslogg.Aktivitet.Severe
...
open class Aktivitetslogg(
    ...
    private val kontekster =
        mutableListOf<KontekstLogable>() // Doesn't need serialization

    internal fun MutableList<Kontekst>.snapshot(): List<Kontekst> =
        this.toList()
    ...
    private fun MutableList<KontekstLogable>.toKontekst() =
        this.map { it.opprettKontekst() }
    ...
    override fun kontekster(): List<Aktivitetslogg> {
        ...
        val aktivitetListeListe: List<List<Aktivitet>> =
            groupBy.map { it.value }
    ...
    private fun warn() =
        Aktivitet.Warn.filter(aktiviteter)

    override fun behov() =
        Aktivitet.Behov.filter(aktiviteter)

    private fun error() =
        Aktivitet.Error.filter(aktiviteter)

    private fun severe() =
        Aktivitet.Severe.filter(aktiviteter)
    ...
    sealed class Aktivitet(
        ...
        operator fun contains(kontekst: KontekstLogable) =
            kontekst.opprettKontekst() in kontekster
